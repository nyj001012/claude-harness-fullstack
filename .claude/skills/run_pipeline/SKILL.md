...
1. **팀원 간 직접 통신 (P2P Communication)**
   ...
   - ⭐️ **Fast 트랙에는 팀 모드가 없다.** 구현 1건과 리뷰 1패스를 **서브 에이전트로 순차 스폰**하고, 반려는 오케스트레이터가 중계한다. 핑퐁 빈도가 1~2회뿐이라 팀 모드의 상시 채널 비용을 회수하지 못한다.
...
7. **쓰기 도구 선택 (Edit vs Write)**
   - **기존 파일을 고칠 때는 `Edit`를 쓴다. `Write`는 신규 파일 생성 전용이다.** 오케스트레이터 자신도 예외가 아니다 — 하네스 메타 라우트에서 `.claude/**`를 직접 편집할 때 이 규칙이 적용된다.
...
- ⭐️ **데이터 레인 투입 조건:** 스키마·마이그레이션·시드 변경이 있을 때만 `db-engineer`를 스폰한다. 변경이 없으면 스폰하지 않는다.
  - **선후 관계가 있는 유일한 레인이다.** `backend-developer`의 데이터 접근 계층은 스키마가 확정된 뒤에 맞춰야 하므로, `db-engineer`가 스키마 확정을 `SendMessage`로 알리고 BE 레인이 그 뒤에 데이터 접근 코드를 붙인다. FE 레인은 계약만 보고 병렬로 달린다.
...
- ⭐️ **레인을 더 쪼갤 때는 화면·기능 단위로 같은 타입을 복제한다.** FE 작업량이 커서 병렬화가 필요하면 `frontend-developer-1`·`frontend-developer-2`처럼 Rule 1의 복제 규약을 쓰고, 각 인스턴스에 **서로 겹치지 않는 화면·기능과 그에 대응하는 파일 집합**을 배정한다.
  - ⛔ **관심사(마크업·스타일·이벤트) 축으로 쪼개지 마라.** 그 셋은 같은 파일에 살기 때문에 병렬 쓰기가 충돌하고, 충돌을 되돌리는 병합 단계가 필요해져 결국 직렬화된다. 병합으로 되돌려야 하는 분할은 경계가 아니다. FE ∥ BE 병렬이 성립하는 이유는 계약이 이음선을 만들고 파일 트리가 겹치지 않기 때문이며, 같은 조건을 만족하는 축만 유효하다.
...
- ⭐️ **Fast 트랙의 Phase 3:** Track A/B의 팀 모드를 쓰지 않고 아래 순서로만 진행한다.
  1. 타겟 파일을 담당하는 구현 역할 **1개**를 서브 에이전트로 스폰한다. 스폰 프롬프트에 타겟 파일과 완료 조건을 명시하고 그 밖의 경로를 건드리지 않게 경계를 재확인시킨다.
  2. `code-reviewer`를 서브 에이전트로 스폰해 **단일 패스** 검수를 받는다. 이 게이트는 생략하지 않는다.
