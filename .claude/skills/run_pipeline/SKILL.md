...
- ⭐️ **라우트 판별 (주입보다 먼저 수행):** 어떤 선행 명령을 실행할지가 라우트에 따라 갈리므로, 주입·검사에 앞서 라우트를 먼저 확정한다.
  ...
  - **하네스 메타 (Harness-meta):** Phase 2(티켓·브랜치) ➔ Phase 3(오케스트레이터 직접 편집) ➔ Phase 5. Phase 1과 Phase 4는 수행하지 않는다.
    ...
    - **판별 1단계 — 이 저장소가 하네스 자체인가:** 루트 파일이 하네스 자산인지는 저장소의 정체에 달려 있다. 아래 두 신호가 **모두** 참일 때만 하네스 저장소로 본다.
      - `bin/cli.mjs`가 존재한다. 스캐폴더는 `.claude/` 하위만 대상 프로젝트로 복사하므로 이 파일은 하네스 저장소에만 있다. 그래서 유효한 식별자다.
      - `package.json`의 `name`이 `claude-harness-fullstack`이다.
    - **판별 2단계 — 변경 대상이 전부 하네스 자산인가:** 자산의 범위가 문맥에 따라 다르다.

      | 문맥 | 하네스 자산의 범위 |
      |---|---|
      | 하네스 저장소 (1단계 통과) | `.claude/` 하위(`agents/`·`skills/`·`tools/`·`settings*.json`) **+** 루트의 배포·문서 자산(`package.json`·`bin/`·`LICENSE`·`README.md`·`.gitignore`·`.gitattributes`) |
      | 대상 프로젝트 (1단계 불통과) | **`.claude/` 하위만.** 루트 파일은 애플리케이션 자산이다 |
    ...
    - ⛔ **이 라우트에서는 `inject-design.mjs`를 실행하지 않는다.** 주입도, `--sections` 선행 검사도 건너뛴다. 하네스는 스택 무관이 설계 원칙이므로 하네스 자체를 손볼 때 `design.md` 부재는 결함이 아니라 정상 상태다. 그 상태에서 주입을 실행하면 추적 대상인 `.claude/agents/*.md` 10개에 `[NOT READY]` 블록이 기록돼 작업트리만 오염된다. 주입할 명세도, 차단해야 할 스택 의존 작업도 없다.
