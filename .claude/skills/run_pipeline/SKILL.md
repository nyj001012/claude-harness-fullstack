...
- ⭐️ **라우트 판별 (주입보다 먼저 수행):** 어떤 선행 명령을 실행할지가 라우트에 따라 갈리므로, 주입·검사에 앞서 라우트를 먼저 확정한다.
  ...
  - **하네스 메타 (Harness-meta):** Phase 2(티켓·브랜치) ➔ Phase 3(오케스트레이터 직접 편집) ➔ Phase 5. Phase 1과 Phase 4는 수행하지 않는다.
    - **판별 기준:** 변경 대상이 전부 `.claude/` 하위의 하네스 자산(`agents/`·`skills/`·`tools/`·`settings*.json`)이고 애플리케이션 소스·테스트·인프라 파일을 한 건도 건드리지 않는 작업이다. 하네스 자체의 정책·문서·도구를 손보는 작업이 여기에 해당한다. 두 영역이 섞인 작업은 하네스 메타가 아니므로 아래 주입 절차를 정상 수행한다.
